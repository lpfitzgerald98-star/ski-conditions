...
- **Grade commentary (done):** one plain-language sentence or two under each
  scorecard's letter grade ("Grading an A: 22 inches of new snow over the past
  seven days. And there's a storm on the board: about 14 inches forecast over
  the next couple of days."). Two interchangeable generators write the SAME
  `card["commentary"]` field, chosen by `config.COMMENTARY_MODE` (or the
  `COMMENTARY_MODE` env var):
  - **`"rules"` — ACTIVE default (`ski/commentary_rules.py`).** Pure Python:
    deterministic templates assembled from clause pools (lead driver / forecast /
    caveat), varied per mountain by a seeded RNG. No API call, no key, no
    external dependency. This is what renders on the live site today.
  - **`"ai"` — built and ready, not active (`ski/commentary.py`).** One
    `claude-opus-4-8` call per mountain per day, cached in SQLite; the model
    phrases the card's own numbers, never measures. To switch back: add an
    `ANTHROPIC_API_KEY` secret to the deploy environment and set
    `COMMENTARY_MODE=ai` (one env var — no code change). Without the key it
    silently yields null, which is why the default is `"rules"`.

  Both paths share the same off-season gate (`commentary.facts_from_card`
  returns `None`, so no prose for a bare/unknown/unscored mountain).
- **Medium-range forecast tier (done):** the forecast now reaches 4-10 days out,
  not just 24/48/72h. `ski/sources/outlook.py`'s `MediumRangeBand` reports a
  low/mid/high accumulation RANGE (not a point estimate) over whatever window
  the source actually covers — NWS reports its real ~7-day reach, Open-Meteo
  reaches the full ~10 days — plus a confidence that tapers as the window
  widens. Folds into the existing forecast blend at a small, distance-tapered
  weight (`config.MEDIUM_RANGE`, 12% max) so near-term still dominates; surfaced
  on the card as `outlook.medium_range`.
- **Global / Regional Comparable Score (done):** `ski/comparable.py` answers
  "where should I go ski RIGHT NOW" as a distinct question from the existing
  `overall`/`region_score`, which are partly self-relative (a resort's best
  season ever can outrank another's ordinary one). `global_score` and
  `regional_score` percentile-rank four ABSOLUTE, cross-mountain-comparable
  inputs — current base depth, trailing 3-day fresh snow, season-to-date, and
  the 72h forecast total — against the whole roster or just one region
  (`config.GLOBAL_SCORE_WEIGHTS`: base .35 / fresh .30 / season .20 / forecast
  .15). Same in-season gate as the rest of the codebase; nothing existing was
  renamed or removed. `GET /scores` gained a `sort` param (default
  `global_score`). Known limitation, not solved here: absolute inches aren't
  perfectly comparable across climates (snow density) or station sitings
  (valley vs. summit) — flagged in `config.GLOBAL_SCORE_WEIGHTS`'s docstring
  for a future pass.
- **Grade stability (done):** the OVERALL letter (map pin / card headline) no
  longer flaps between adjacent grades on day-to-day noise near a boundary.
  `ski/stability.py` adds a hysteresis band: once a grade is set, the raw value
  must clear the boundary by `config.GRADE_HYSTERESIS_MARGIN` (2.5 points, in
  the direction of the move) before the letter actually flips to an adjacent
  grade; a jump big enough to skip a grade (a real storm) is never held back.
  One small persisted table (`grade_stability`, keyed by mountain/profile/date)
  anchors each day to the last stable grade within
  `GRADE_HYSTERESIS_LOOKBACK_DAYS` (5) — distinct from `ski/cache.py`'s pure
  render cache, which must never feed grading; this table's entire purpose is
  the opposite. Live scoring only — `retro=True` historical dates are immutable
