# Glade Grade — Snow Quality / Skiability Improvement Plan

**Status:** Approved 2026-07-17. Audit + design done (Opus). Implementation in progress.

## Progress (2026-07-17)

- **Phase 0 — DONE.** `SnowQuality` scaffold surfaced on the card, weight 0.
- **Phase 1 — DONE.** Commentary linkage: quality signals in `facts` (serves rules + AI),
  quality-drag lead branch, de-dup, conservative positive note.
- **Phase 2 — DONE.** New-snow density, tiered (measured ΔSWE/Δdepth on 37 SNOTEL+Mammoth;
...
- **Phase 3 — DONE.** Wind loading/scour (magnitude-only), from Open-Meteo hourly wind. Wind
...
- **Phase 4 — DONE.** SnowQuality folded into the comparable leaderboard as a 5th weighted
...
- **Phase 5a — DONE.** Continuous powder recency decay (POWDER_DECAY) replacing the
...
- **Phase 5b — DONE (SNOTEL-only, user chose to build it).** Buried rain/melt crust from the
...
- **Phase 6 — DONE.** `OVERALL_GRADE_THRESHOLDS` untouched (overall path unchanged). Skiability
  distribution re-checked offline (roster × 3 winter dates, n=247): median 43.6, A+ 3.6%, smooth
  spread, D/F tail 6.5% — thresholds still valid, no change. Live-only shift (wind/thaw/Tier-2
...
Audit finding (important correction to the original premise): a quality layer **already exists** —
...
2. **Two quality signals missing despite data on hand:** snow **density (SWE:depth)** — SNOTEL stores
   both `WTEQ` and `SNWD` daily but the ratio is only used as a hardcoded `3.0` unit constant; and
   **wind loading/scour** — not ingested for the surface at all.
...
Introduce a single explicit **`SnowQualityScore` (0–100)** computed once in the pipeline, with named
components (`density`, `wind`, `crust`, `thaw`, `warmth`). It:
- feeds the skiability `quality_factor` as today (**multiplicative** — preserves existing tuning),
- becomes a **weighted component** in the comparable global/regional blend (**additive** — so quality
...
6. **Re-tune:** re-backtest grade thresholds after the distribution shifts — its own pass.

## Phases (each gated on confirm-before-building)

- **Phase 0 — scaffold + observability.** `SnowQualityScore` computed from existing signals, surfaced
  on the card, **weight 0** in all consumers. No grade change. Look before it's load-bearing.
- **Phase 1 — commentary linkage (B1).** Pipe `quality_factor`/`refreeze`/`thaw` into `facts_from_card`
  (serves rules + future AI). Add quality-driven lead branch in `commentary_rules._lead`.
- **Phase 2 — snow density (B2).** Derive storm-window SWE:depth from `raw_observations`; backfill via
...
- **Phase 3 — wind loading/scour (B3), magnitude-only.** Add Open-Meteo wind (forecast + ERA5 history).
  Sustained wind on fresh snow → penalty. Store direction for later aspect work. Add wind clause.
- **Phase 4 — leaderboard fold (B4) + floor.** Add `SnowQualityScore` as weighted component in
  `comparable`, renormalizing over available components. Ramp moderate→strong. Lower skiability floor.
- **Phase 5 — recency decay (B5) + rain-on-snow persistence (B6).** Continuous age decay modulated by
  preservation; buried-crust memory from season rain-on-snow history.
- **Phase 6 — re-backtest thresholds.** Re-tune `OVERALL_GRADE_THRESHOLDS` /
  `SKIABILITY_GRADE_THRESHOLDS` against the quality-shifted distribution.

**Deferred:** B7 (sun/aspect exposure) — needs new per-resort aspect data. Revisit only if B1–B5 fall short.
